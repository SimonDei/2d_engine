...
namespace sde {
	...
	const SubTexture& TextureSheet::get_texture_rect(float x, float y, float width, float height) {
		...
		m_sub_bitmaps.insert(std::make_pair("x_" + std::to_string(x) + "y_" + std::to_string(y) + "w_" + std::to_string(width) + "h_" + std::to_string(height), SubTexture(m_bitmap, x, y, width, height)));
